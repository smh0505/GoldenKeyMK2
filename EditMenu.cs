...
namespace GoldenKeyMK2
{
    public class EditMenu
    {
        ...
        public static void DrawEdit()
        {
            ...
            {
                before = _idx == 0 ? Wheel.Options.Count - 1 : _idx - 1;
                after = _idx == Wheel.Options.Count - 1 ? 0 : _idx + 1;
            }
            ...
            Raylib.DrawRectangle(64, 184, 752, 48, Wheel.Options.Count > 0 ? Raylib.Fade(Wheel.Options[_idx].Color, 0.5f) : Color.DARKGRAY);
        ...
        public static void Control()
        {
            switch ((KeyboardKey)Raylib.GetKeyPressed())
            {
                case KeyboardKey.KEY_UP:
                    if (Wheel.Options.Count > 0) _idx = _idx == 0 ? Wheel.Options.Count - 1 : _idx - 1;
                    break;
                case KeyboardKey.KEY_DOWN:
                    if (Wheel.Options.Count > 0) _idx = _idx == Wheel.Options.Count - 1 ? 0 : _idx + 1;
                    break;
            }
        }
    }
